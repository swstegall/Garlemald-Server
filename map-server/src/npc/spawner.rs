...
impl SpawnContext<'_> {
    ...
    pub async fn spawn_zone(&self, zone_id: u32) -> Vec<u32> {
        ...
        //
        // Root + private-area seeds both spawn here. The root-only
        // `z.spawn_locations` clone this used to take left every
        // private-area seed unmaterialised — the SEQ_010 quest NPCs
        // (Farrimond/Cecilia/Swethyna/Tkebbe/Lonsygg + blockers, zone 155
        // 'PrivateAreaMasterPast' level 1) never existed, so the man0g0
        // post-tutorial warp landed in an empty Gridania (zone-in scan:
        // count=0) and every SEQ_010 SetENpc broadcast was skipped.
        // `spawn_one` routes each seed's grid insert into the private
        // area's own core, so cross-level phases (level 1 vs the Man0g1
        // level-2 townsfolk) stay mutually invisible. (Garlemald #28.)
        let (seeds, mut next_number) = {
            let z = zone_arc.read().await;
            let seeds: Vec<SpawnLocation> =
                z.all_spawn_locations().map(|(_, _, s)| s.clone()).collect();
...
async fn spawn_one(
    ...
    // 2. Push a spatial projection into the zone. Private-area seeds
            ...
            match zone.get_private_area_mut(&seed.private_area_name, seed.private_area_level) {
                ...
                None => {
                    tracing::warn!(
                        ...
                        area = %seed.private_area_name,
                        level = seed.private_area_level,
